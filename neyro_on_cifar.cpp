...
double dE_rms_g1[meadle_size][in_size], dE_rms_g2[out_size][meadle_size], dE_rms_b3[out_size], dE_rms_b2[meadle_size];
...
void Ini() 
{
	...
	memset (dE_g1, 0.1, sizeof(dE_g1));//массив экспоненциального забывания по градиенту(производной в квадрате) весов для  1->2 слой
    memset (dE_g2, 0.1, sizeof(dE_g2));//массив экспоненциального забывания по градиенту весов для  2->3 слой
	memset (dE_b3, 0.1, sizeof(dE_b3));//массив экспоненциального забывания по градиенту сдвига b на 3м слое
    memset (dE_b2, 0.1, sizeof(dE_b2));//массив экспоненциального забывания по градиенту сдвига b на 2м слое
...
int main()
{
  ...
  //  Ini();//инициальзация вектором w и сум//в инициализацию так же ушло чтение базы данных
	...
	int res = 0, res2 = 0, ans = 0, type_teach = 7;
	bool flag_read = true;
    ...
    while (ans != 4) 
    {
        ...
        case 1://обучение
            ...
            { 
				//if (number_step == 20) type_teach = 1;
				//if (number_step == 40) type_teach = 4;
				...
				if ((error_arr[0] < error_arr[1] )&& (error_arr[1] < error_arr[2]) && (error_arr[2] < error_arr[3])&& (number_step>=70)) // если ошибка некоторое время увеличивается - окончание обучения
					stop = false;
                number_step++;
