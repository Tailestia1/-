...
void Ini() 
{
	...
	memset (dE_g1, 0, sizeof(dE_g1));
    memset (dE_g2, 0, sizeof(dE_g2));
	memset (dE_b3, 0, sizeof(dE_b3));
    memset (dE_b2, 0, sizeof(dE_b2));
	...
	for (int i = 0; i < meadle_size; i++)//инициализация весов из первого слоя во второй и сдвига нейронов на внутреннем слое
	{
		...
		{
			weight1[i][j] = 0.1 *  rand() / RAND_MAX ;//(rand() % 100) / 100.0 / 5.0 - 0.1;//(определение некоторого рандома, но не знаю почему в таком виде)
	...
	for (int i = 0; i < out_size; i++)//инициализация весов из внутреннего слоя в третий, выходной и сдвига нейронов на внешнем слое
	{
		...
		{
            weight2[i][j] = 0.1 * rand() / RAND_MAX ;// (rand() % 100) / 100.0 / 5.0 - 0.1;
...
int main()
{
    ...
    while (ans != 4) 
    {
       ...
       // double error_arr[4] = {9999,9999,9999,9999}, error_of_step = 0, err_of_res=0;
        ...
        {
        case 1:
			for (type_teach = 3; type_teach < 7; type_teach++)
			{
				...
				case 1: gr_err = fopen("graf_of_error1.csv", "w"); Ini(); break;
				case 2: gr_err = fopen("graf_of_error2.csv", "w"); Ini(); break;
				case 3: gr_err = fopen("graf_of_error3.csv", "w"); Ini(); break;
				case 4: gr_err = fopen("graf_of_error4.csv", "w"); Ini(); break;
				case 5: gr_err = fopen("graf_of_error5.csv", "w"); Ini(); break;
				case 6: gr_err = fopen("graf_of_error6.csv", "w"); Ini(); break;
				default: break;
				}
				//z1 = fopen("weight.csv", "w");
				while (stop && number_step < steps)  // эпохи обучения. степ - максимально количество эпох
				{
					...
					if ((error_arr[0] < error_arr[1]) && (error_arr[1] < error_arr[2]) && (error_arr[2] < error_arr[3]) && (number_step >= 100)) // если ошибка некоторое время увеличивается - окончание обучения
						stop = false;
					number_step++;
