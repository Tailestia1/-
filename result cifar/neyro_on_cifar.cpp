...
*/
const int steps = 30, w_img=32;// steps - максимальное количество эпох обучения.
const int in_size = w_img*w_img*3, meadle_size = 40, out_size = 10;//размеры сетки: вход, внутренний, выходной (картинку вытянем из мтарицы в строку длины 784)
const int valid_size = 5000, test_size = 10000, base_size = 30000;//укороченный вариант
...
const double n = pow(1.0, -4),//0.0001,// скорость обучения,
...
/*void sobel(double *Y, double *Y_new,  int T)//Т-порог при котором если gradf>T I=255 rgadf<T I=0. выделение границ
{
	int i, j, i1, j1, im;
	int Mx[9] = { -1, 0, 1, -2, 0, 2, -1, 0, 1 },
		My[9] = { 1, 2, 1, 0, 0, 0, -1, -2, -1 };
	double Gx = 0, Gy = 0, gradf = 0;
	double Y_L[w_img*w_img];
	for (i = 0; i < w_img; i++)
		for (j = 0; j < w_img; j++)
		{//это для какой компоненты вычисляем. далее циклы что вычисляем 
			Gx = Gy = 0;//обнуляем сумму посчитанных элементов
			for (i1 = i - 1, im = 0; (i1 <= i + 1) && (i1 < w_img); i1++)
				for (j1 = j - 1; (j1 <= j + 1) && (j1 < w_img); j1++, im++)
				{
					if (i1 < 0 || j1 < 0) continue;
					Gx += Y[i1 * w_img + j1] * Mx[im];
				}
			for (i1 = i - 1, im = 0; (i1 <= i + 1) && (i1 < w_img); i1++)
				for (j1 = j - 1; (j1 <= j + 1) && (j1 < w_img); j1++, im++)
				{
					if (i1 < 0 || j1 < 0) continue;
					Gy += Y[i1 * w_img + j1] * My[im];
				}
			gradf = sqrt(Gx*Gx + Gy * Gy);
			if (gradf >T) Y_L[i*w_img + j] = 1;
			else Y_L[i*w_img + j] = 0;
		}
	for (i = 0; i < w_img*w_img; i++)
		Y_new[i] = Y_L[i];
}*/
...
{//название файла, так как там имеется 5 файлов с данными, курр - маркер куда записывать данные, tr_test - маркет Train база(0) или Test(1) база
    ...
    if (file.is_open()){
		for(int i = 0; i < test_size; ++i)
        {
			...
			//sobel(int_input[curr + i], int_input[curr + i], 50);
			//*/
			...
			/*for (int j = 0; j < w_img*w_img; j++)
			{
				file.read((char*)&temp, sizeof(temp));//b
				int_input[curr + i][j] = (double)temp / 255.0;
				file.read((char*)&temp, sizeof(temp));//g
				int_input[curr + i][w_img * w_img + j] = (double)temp / 255.0;
				file.read((char*)&temp, sizeof(temp));//r
				int_input[curr + i][w_img * w_img * 2 + j] = (double)temp / 255.0;
			}*/
		}
		file.close();
	}
...
void Ini() 
{
	...
	memset (dE_g1, 0, sizeof(dE_g1));//массив экспоненциального забывания по градиенту(производной в квадрате) весов для  1->2 слой
    memset (dE_g2, 0, sizeof(dE_g2));//массив экспоненциального забывания по градиенту весов для  2->3 слой
	memset (dE_b3, 0, sizeof(dE_b3));//массив экспоненциального забывания по градиенту сдвига b на 3м слое
    memset (dE_b2, 0, sizeof(dE_b2));//массив экспоненциального забывания по градиенту сдвига b на 2м слое
...
{//сейчас реализовано RMS и накопление пачками
	...
	case 7:
		for (int h = 0; h < meadle_size; h++)//номер внутреннего слоя
		{
			...
			dE_b2[h] = gamma * dE_b2[h] + (1 - gamma) * dw2 * dw2;//экспоненциальное забывание для сдвига
			if (time < 6)
				d_b2[h] += dw2;// / sqrt(dE_b2[h] + eps);//накопления для bd_b2[h] += dw2;//накопления для b
			else
				d_b2[h] += dw2 / sqrt(dE_b2[h] + eps);
			...
			for (int i = 0; i < in_size; i++)//обновление по всем весам внешнего слоя связанных с конкретным нейроном внутреннего слоя
			{
				...
				if ( time < 6)  d_weight1[h][i] += dw;
				else d_weight1[h][i] += dw / sqrt(dE_g1[h][i] + eps);
		...
		for (int m = 0; m < out_size; m++)//по количеству внешних нейронов
		{
			dw2 += (out[m] - answer[g][m]) * (1 - out[m]) * out[m];//значение для обновления b , dw=dw2*act(h)-для весов
			dE_b3[m] = gamma * dE_b3[m] + (1 - gamma) * dw2 * dw2;//экспоненциальное забывание для B внешнего слоя
			if( time < 6) d_b3[m] += dw2;//накопление суммы градиента
			else d_b3[m] += dw2 / sqrt(dE_b3[m] + eps);
			...
			for (int h = 0; h < meadle_size; h++)//h-внутенний слой (веса откуда), для весов связанных с конкретным m
			{
				dw = dw2 * act[h];//частная производная для wmh(в m из h)
				dE_g2[m][h] = gamma * dE_g2[m][h] + (1 - gamma) * dw * dw;
				if (time < 6) d_weight2[m][h] += dw;
				else d_weight2[m][h] += dw / sqrt(dE_g2[m][h] + eps);
				if (d_count == d_count_max)
				{
					weight2[m][h] -= n * d_weight2[m][h] / (1.0 * d_count_max);//изменения веса.
					d_weight2[m][h] = 0;
    ...
    while (ans != 4) 
    {
        ...
        case 1://обучение
            ...
            while (stop && number_step < steps)  // эпохи обучения. степ - максимально количество эпох
            {
				...
				if ((error_arr[0] < error_arr[1] )&& (error_arr[1] < error_arr[2]) && (error_arr[2] < error_arr[3])&& (number_step>=250)) // если ошибка некоторое время увеличивается - окончание обучения
					stop = false;
                number_step++;
