...
*/
const int steps = 300, w_img=32;// steps - максимальное количество эпох обучения.
...
void Ini() 
{
	...
	/*for (int i = 0; i < meadle_size; i++)//инициализация весов из первого слоя во второй и сдвига нейронов на внутреннем слое
	{
		...
		for (int j = 0; j < in_size; j++)
		{
			weight1[i][j] = 0.001 *  rand() / RAND_MAX ;//(rand() % 100) / 100.0 / 5.0 - 0.1;//(определение некоторого рандома, но не знаю почему в таком виде)
	...
	for (int i = 0; i < out_size; i++)//инициализация весов из внутреннего слоя в третий, выходной и сдвига нейронов на внешнем слое
	{
		...
		{
            weight2[i][j] = 0.001 * rand() / RAND_MAX ;// (rand() % 100) / 100.0 / 5.0 - 0.1;
...
{//сейчас реализовано RMS и накопление пачками
	...
	case 0://обновления без ускоряющих методов
		for (int h = 0; h < meadle_size; h++)//номер внутреннего слоя
		{
			...
			for (int m = 0; m < out_size; m++)
				dw2 += (out[m] - answer[g][m]) * weight2[m][h];
			dw2 *= (1 - act[h]) * act[h] / x;// (in_size*out_size);//посчитали часть производной для обновления весов и сдвигов. dw2 можно использовать для b, dw2*input для весов в зависимости от
		...
		{
			dw2 += (out[m] - answer[g][m]) / x;// (in_size*out_size);//значение для обновления b , dw=dw2*act(h)-для весов
	...
	case 2://adam
		...
		for (int h = 0; h < meadle_size; h++)//номер внутреннего слоя
		{
			...
			for (int m = 0; m < out_size; m++)
				dw2 += (out[m] - answer[g][m]) * weight2[m][h];
			dw2 *= ((1 - act[h]) * act[h]) / x;///(in_size*out_size);//посчитали часть производной для обновления весов и сдвигов. dw2 можно использовать для b, dw2*input для весов в зависимости от индекса инпута
			//построение оценки
	...
	case 5://nesterov есть вопросы в применимости в данном случае
		...
		for (int m = 0; m < out_size; m++)//по количеству внешних нейронов
		{
			dw2 = (out[m] - answer[g][m]) * (1 - out[m]) * out[m]/x;//значение для обновления b , dw=dw2*act(h)-для весов
			dE_b3[m] = gamma * dE_b3[m] + n* dw2;//экспоненциальное забывание для B внешнего слоя
			d_b3[m] += dE_b3[m];
		}
		Set_Networkw1(g);
...
double Error(int g) 
{
    double s = 0;
    for (int i = 0; i < out_size; i++)
			s+= answer[g][i] * log(out[i]) + (1 - answer[g][i]) * log(1 - out[i]);
	s = (-1)*s;
    return s;
}
...
int main()
{
  ...
  //  Ini();//инициальзация вектором w и сум//в инициализацию так же ушло чтение базы данных
	FILE *gr_err, *analiz_file;// , *weight_file;
